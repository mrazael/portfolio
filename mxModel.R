...
modelCIs <- c(CI_cor, CI_std, CI_her)
...
CholACTEModel<- mxModel( "CholACTE", pars, modelMZ, modelDZ, multi, minus2ll, obj, modelCIs)
#---------------------------------------------------------------------------------------------------------------------
# RUN Cholesky Decomposition ACTE Model
...
mxCompare( CholACTEFit, CholACTEhypoFit)
omxAkaikeWeights(models = c(CholACTEFit, CholACTEhypoFit), type = c("AIC", "AICc"), conf.level = 0.95)
...
#BOOTSTRAP MODELS FOR BETTER CONFIDENCE INTERVALS
set.seed(666)
ACTEboot <- mxBootstrap(CholACTEhypoFit, replications = 2000, verbose = 2L)
ACTEevalstda <- mxBootstrapEvalByName(name = "std_a", model = ACTEbootPoor, bq = c(0.05, 0.95), replications = 10000, verbose = 2L)
